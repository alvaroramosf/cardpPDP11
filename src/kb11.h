...
class KB11 {
  public:
    ...
    constexpr inline uint16_t currentmode() { return (PSW >> 14) & 3; }
  ...
  private:
    ...
    template <int len> inline uint16_t DA(const uint16_t instr) {
    ...
    template <int len> uint16_t fetchOperand(const uint16_t instr) {
    ...
    template <int len> constexpr uint16_t SS(const uint16_t instr) {
        static_assert(len == 1 || len == 2);
        if (!((instr >> 6) & 070)) {
    ...
    template <int l> constexpr inline uint16_t read(const uint16_t a) {
    ...
    template <int l> constexpr void write(const uint16_t a, const uint16_t v) {
        static_assert(l == 1 || l == 2);
        auto vl = v;
    ...
    template <int l> constexpr inline uint16_t max() {
        static_assert(l == 1 || l == 2);
        if constexpr (l == 2) {
            return 0xffff;
        } else {
    ...
    template <int l> constexpr inline uint16_t msb() {
        static_assert(l == 1 || l == 2);
        if constexpr (l == 2) {
            return 0x8000;
        } else {
    ...
    template <int l> void CMP(const uint16_t instr) {
    ...
    template <int len> inline void setNZ(const uint16_t v) {
        static_assert(len == 1 || len == 2);
        PSW &= (0xFFF0 | FLAGC);
        if (v & msb<len>()) {
            PSW |= FLAGN;
    ...
    template <int len> inline void setNZV(const uint16_t v) {
        setNZ<len>(v);
        if (v == msb<len>()) {
            PSW |= FLAGV;
        }
    ...
    template <int len> inline void setNZC(const uint16_t v) {
        static_assert(len == 1 || len == 2);
        PSW &= 0xFFF0;
        if (v & msb<len>()) {
            PSW |= FLAGN;
    ...
    template <int l> void BIC(const uint16_t instr) {
        const auto src = SS<l>(instr);
        const auto da = DA<l>(instr);
        const auto dst = read<l>(da);
    ...
    template <int l> void BIS(const uint16_t instr) {
        const auto src = SS<l>(instr);
        const auto da = DA<l>(instr);
        const auto dst = read<l>(da);
        auto uval = src | dst;
    ...
    template <int l> void CLR(const uint16_t instr) {
        PSW &= 0xFFF0;
        PSW |= FLAGZ;
    ...
    template <int l> void COM(const uint16_t instr) {
        const auto da = DA<l>(instr);
        const auto dst = ~read<l>(da);
        write<l>(da, dst);
        PSW &= 0xFFF0;
    ...
    template <int l> void _DEC(const uint16_t instr) {
        const auto da = DA<l>(instr);
    ...
    template <int l> void NEG(const uint16_t instr) {
        const auto da = DA<l>(instr);
        ...
        template <int l> void _ADC(const uint16_t instr) {
        const auto da = DA<l>(instr);
        auto uval = read<l>(da);
        if (PSW & FLAGC) {
    ...
    template <int l> void SBC(const uint16_t instr) {
        const auto da = DA<l>(instr);
        auto sval = read<l>(da);
        auto qval = sval;
    ...
    template <int l> void ROR(const uint16_t instr) {
        const auto da = DA<l>(instr);
        const auto dst = read<l>(da);
        auto result = dst >> 1;
        if (PSW & FLAGC) {
    ...
    template <int l> void ROL(const uint16_t instr) {
        const auto da = DA<l>(instr);
    ...
    template <int l> void ASR(const uint16_t instr) {
        const auto da = DA<l>(instr);
        auto uval = read<l>(da);
        PSW &= 0xFFF0;
        if (uval & 1) {
    ...
    template <int l> void ASL(const uint16_t instr) {
        const auto da = DA<l>(instr);
    ...
    template <int l> void INC(const uint16_t instr) {
        const auto da = DA<l>(instr);
    ...
    template <int l> void SBIT(const uint16_t instr) {
        const auto src = SS<l>(instr);
    ...
    template <int l> void TST(const uint16_t instr) {
        const auto dst = read<l>(DA<l>(instr));
        PSW &= 0xFFF0;
        if ((dst & max<l>()) == 0) {
            PSW |= FLAGZ;
    ...
    template <int len> void MOV(const uint16_t instr) {
